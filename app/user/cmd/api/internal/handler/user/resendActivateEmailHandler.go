...
func ResendActivateEmailHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.ResendActivateEmailReq
		if err := httpx.Parse(r, &req); err != nil {
			response.ParamError(r.Context(), w, err)
			return
		}

		l := user.NewResendActivateEmailLogic(r.Context(), svcCtx)
		resp, err := l.ResendActivateEmail(&req)
